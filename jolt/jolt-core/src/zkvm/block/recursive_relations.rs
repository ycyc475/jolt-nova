...
pub(super) fn bind_recursive_sumchecks_to_verifier_r1cs<CS: ConstraintSystem<NovaScalar>>(
    ...
    let rounds = stages
        ...
        .collect::<Vec<_>>();
    let regular_grid_rows = if verifier_r1cs.stage_configs.is_empty() {
        // Stage-14/15 test fixtures predate explicit stage metadata. Production
        // artifacts always carry configs generated by VerifierR1CSBuilder.
        (0..verifier_r1cs.hyrax.total_rounds).collect::<Vec<_>>()
    } else {
        verifier_r1cs
            .stage_configs
            .iter()
            .scan(0usize, |row_offset, config| {
                let start = *row_offset;
                *row_offset += config.num_rounds;
                Some((config.is_uniskip(), start, config.num_rounds))
            })
            .filter(|(is_uniskip, _, _)| !is_uniskip)
            .flat_map(|(_, start, num_rounds)| start..start + num_rounds)
            .collect::<Vec<_>>()
    };
    if rounds.len() != regular_grid_rows.len()
    ...
    for (artifact_round_index, (round_index, coefficients)) in
        regular_grid_rows.into_iter().zip(rounds).enumerate()
    {
        ...
        for column in 0..verifier_r1cs.hyrax.C {
            ...
            if let Some(coefficient) = coefficients.get(column) {
                cs.enforce(
                    || {
                        format!(
                            "bind sumcheck artifact round {artifact_round_index} to grid row {round_index} coefficient {column}"
                        )
                    },
                ...
                cs.enforce(
                    || {
                        format!(
                            "zero-pad sumcheck artifact round {artifact_round_index} at grid row {round_index} coefficient {column}"
                        )
                    },
